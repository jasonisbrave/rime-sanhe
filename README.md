...
> **下载安装包 v1.0.0** → [GitHub 直链](https://github.com/jasonisbrave/rime-sanhe/releases/download/v1.0.0/rime-sanhe-v1.0.0.zip) · [GitHub Releases](https://github.com/jasonisbrave/rime-sanhe/releases) · [GitCode Releases](https://gitcode.com/jasonisbrave/rime-sanhe/releases)（源码归档镜像，克隆本仓库亦可直接部署）
...
- 带调全拼词库，支持声调辅助筛选：`7`/`8`/`9`/`0` 对应一/二/三/四声，连按自动覆盖（不确定声调就 7890 连按）
- 智能纠错：错音、错字兼容（如 `zhidao`→知道、`jandan`→简单）
- 拆字与笔画反查：输入 `` ` `` 进入拆字反查
- 中英混输：直接打英文单词、自动识别
- 拆分输入、超级注释、时间日期（`\rq`）、计算器、符号大全（`/fh`）等 Lua 增强
- 五笔召唤（beta）：拼音流输入 `/w+编码`（如 `/wvbg`）直接出五笔候选，注释显示完整编码，边打边学码
