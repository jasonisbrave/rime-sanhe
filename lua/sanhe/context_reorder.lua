...
function P.init(env)
    ...
    env.is_t9 = sanhe.get_input_method_type and sanhe.get_input_method_type(env) == "t9" or false
    env.undo_before = {}
    env.undo_after = {}
    clear_undo(env)
...
function P.func(key, env)
    ...
    if key:release() then return sanhe.RIME_PROCESS_RESULTS.kNoop end
...
function F.func(input, env)
    ...
    if current_input == "" or sanhe.is_special_mode(ctx) then
        clear_learning_snapshot()
        for cand in input:iter() do yield(cand) end
        return
    end
