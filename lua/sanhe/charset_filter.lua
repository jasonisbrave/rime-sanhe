...
-- 借 sanhe 共享 charset_filter 运行态
local runtimes = sanhe._cf_envs or setmetatable({}, { __mode = "v" })
sanhe._cf_envs = runtimes
local masks = {}
...
local function get_charset_db(env)
    ...
    local fname = dist == "weasel"
        ...
        or sanhe.get_filename_with_fallback("lua/data/charset.reverse.bin")
...
local function codepoint_is_valid(env, codepoint, active_rules, cache)
    ...
    if not sanhe.IsChineseCharacter(char) then
...
local function get_active_rules(env, ctx)
    ...
    if sanhe and sanhe.is_special_mode
        and sanhe.is_special_mode(ctx)
    then
        return nil
    end
