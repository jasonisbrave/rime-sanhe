...
function M.init(env)
    ...
    env._cpc_key_handler = function(key)
        ...
        if not n then return sanhe.RIME_PROCESS_RESULTS.kNoop end
        ...
        if not spans or count == 0 then return sanhe.RIME_PROCESS_RESULTS.kNoop end

        local vertices = type(spans.vertices) == "function" and spans:vertices() or spans.vertices
        if not vertices or #vertices < 2 then return sanhe.RIME_PROCESS_RESULTS.kNoop end
        ...
        if #stops < 2 then return sanhe.RIME_PROCESS_RESULTS.kNoop end
        ...
        if n <= 0 then return sanhe.RIME_PROCESS_RESULTS.kNoop end
        -- 获取需要上屏的中文候选字串
