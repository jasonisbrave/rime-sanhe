...
T（Translator）
调用 sanhe_english 词典，负责英文候选输出、候选数量限制，
...
local function should_skip(env)
    ...
    return sanhe.is_special_mode(ctx)
end
...
function T.init(env)
    ...
    if config then
        local configured_limit = config:get_int("sanhe_english/max_candidates")
    ...
    env.eng_translator = Component.TableTranslator(env.engine, "sanhe_english", "table_translator")
end

function T.func(input, seg, env)
    if (env.engine.schema.schema_id ~= "sanhe_english"
...
function F.init(env)
    ...
    env.memory = env.schema_id == "sanhe_english" and {} or nil
    env.spacing_starts = {}
    env.spacing_chunks = {}
    env.spacing_output = {}
    env.format_parts = {}
    ...
    local delimiter_str = " '"
    if cfg then
        local str = cfg:get_string("sanhe_english/english_spacing")
        if str then
            env.english_spacing_mode = str
        end
        local timeout = cfg:get_double("sanhe_english/spacing_timeout")
        if timeout then
            env.spacing_timeout = timeout
        end
        local key = cfg:get_string("sanhe_lookup/key")
        if key and key ~= "" then
            env.lookup_key = key
        end
        local sym = cfg:get_string("sanhe_english/trigger")
...
function F.func(input, env)
    ...
    for cand in input:iter() do
        ...
        if env.schema_id == "sanhe_english" then
        ...
        if env.schema_id == "sanhe_english"
            and final_comment ~= ""
