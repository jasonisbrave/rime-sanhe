...
local MERGED_SCHEMA_IDS = {"sanhe_pro", "sanhe", "sanhe_lite", "sanhe_english", "sanhe_t9", "sanhe_t9i"}
...
-- 基础依赖
local userdb = require("sanhe/userdb")
...
local function get_file_signature(path)
    local file, close = sanhe.load_file_with_fallback(path, "rb")
...
local function enabled_schema_ids()
    local enabled = {}
    local file, close = sanhe.load_file_with_fallback("build/default.yaml", "r")
...
local function rebuild(tasks, db)
    ...
    for _, task in ipairs(tasks) do
        ...
        local file, close = sanhe.load_file_with_fallback(task.path, "r")
...
local function database_matches(
    ...
    if (db:meta_fetch("_sanhe_ver") or "") ~= current_version
...
local function update_metadata(
    ...
    if not db:meta_update("_sanhe_ver", current_version)
...
function M.init(env)
    ...
    if sanhe and sanhe.version then
        current_version = sanhe.version
    end
    env.input_type = "unknown"
    if sanhe and sanhe.get_input_method_type then
        env.input_type = sanhe.get_input_method_type(env)
    end
