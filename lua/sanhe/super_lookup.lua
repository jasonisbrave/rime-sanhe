...
--sanhe_lookup: #设置归属于super_lookup.lua
--tags: [ abc ]  # 检索当前tag的候选
--key: "`"       # 输入中反查引导符
--lookup: [ sanhe_reverse ] #反查滤镜数据库
...
do
    local ok, mod = pcall(require, "sanhe/charset_filter")
        ...
        for _, db_obj in ipairs(env.db_table) do
            ...
            if res_str and #res_str > 0 then
                for word in res_str:gmatch("%S+") do
                    local cand = Candidate("sanhe_shadow", s_end, #ctx_input, word, "")
                    cand.quality = 1
                    yield(cand)
                end
            end
...
function f.init(env)
    ...
    env.enable_tone = config:get_bool("sanhe_lookup/enable_tone")
    if env.enable_tone == nil then
        env.enable_tone = true
    end

    env.enable_direct = config:get_bool("sanhe_lookup/enable_direct")
    if env.enable_direct == nil then
        env.enable_direct = false
    end

    local sources_list = config:get_list("sanhe_lookup/data_source")
    env.data_sources = {}
    ...
    if env.has_db then
        local db_list = config:get_list("sanhe_lookup/lookup")
...
function f.func(input, env)
    ...
    if s_start then
        ...
        if not env.enable_direct or sanhe.is_pro_scheme(env) then
            env.direct_cache = nil
            for cand in input:iter() do
                yield(cand)
            end
