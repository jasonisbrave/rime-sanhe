...
-- 全局通信通道
_G.SanheSharedState = _G.SanheSharedState
    or {
        sorter_active = false,
        last_input = "",
        page_cache = {},
...
function M.fini(env)
    ...
    -- _G.SanheSharedState 是排序器与本过滤器的通信通道，不能在这里销毁。
end
...
function M.func(input, env)
    ...
    -- PHASE 3: 三码空候选兜底
    if idx == 0 and seg_len == 3 and sub(code, 1, 1) ~= "/" and not sanhe.is_special_mode(ctx) then
