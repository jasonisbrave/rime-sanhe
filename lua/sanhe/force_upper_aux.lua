...
function ForceUpperAux.init(env)
    ...
    env.aux_cache = {}
    env.dict_name = config:get_string("translator/dictionary") or "sanhe_pro"
    env.dict = nil
    ...
    env.on_update = function(ctx)
        ...
        -- 遇到转换模式或功能面板时立刻放行
        if sanhe.is_special_mode(ctx) then
            return
        end
...
function ForceUpperAux.func(key_event, env)
    if key_event:release() then return sanhe.RIME_PROCESS_RESULTS.kNoop end
    ...
    if current_key == env.trigger_key then
        if not ctx:is_composing() then return sanhe.RIME_PROCESS_RESULTS.kNoop end
        ...
        if not found_any_aux then
            env.press_count = 0; env.is_cycling = false; return sanhe.RIME_PROCESS_RESULTS.kNoop
        end
