...
local SCHEME_CAPABILITIES = {
    sanhe_pro = {tone = true, aux = true, pro = true, lite = false, t9 = false},
    sanhe = {tone = true, aux = false, pro = false, lite = false, t9 = false},
    sanhe_lite = {tone = false, aux = false, pro = false, lite = true, t9 = false},
    sanhe_t9 = {tone = true, aux = false, pro = false, lite = false, t9 = true},
    sanhe_t9i = {tone = true, aux = false, pro = false, lite = false, t9 = true},
}
...
function CR.init(env)
    ...
    local file, close_file, err = sanhe.load_file_with_fallback(path)
    if not file then
...
function ZH.init(env)
    ...
    local schema_id = env.engine.schema.schema_id or "sanhe"
    local caps = SCHEME_CAPABILITIES[schema_id] or SCHEME_CAPABILITIES.sanhe
    ...
    if not env.is_t9 and sanhe.get_input_method_type then
        env.input_method_type = sanhe.get_input_method_type(env)
    end
...
function ZH.func(input, env)
    ...
    local is_radical_mode = sanhe.is_in_radical_mode(env)
    local skip_comment = input_str == "" or sanhe.is_function_mode(context)
