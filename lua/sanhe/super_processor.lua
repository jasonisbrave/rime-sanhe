...
local function load_rime_regex_matchers(config, path)
    ...
    for i = 1, #keys do
        ...
        if type(regex) == "string" and regex ~= "" and not seen[regex] then
            local matcher, err = sanhe.compile_regex(regex)
            if matcher then
                seen[regex] = true
...
local function is_function_code_after_digit(env, context, digit_char)
    ...
    for _, matcher in ipairs(matchers) do
        if sanhe.regex_matches(matcher, input) then
            return true
        end
    end
...
function M.init(env)
    ...
    env.sc_last_key = nil
    env.is_t9 = false
    if sanhe.get_input_method_type then
        local im_type = sanhe.get_input_method_type(env)
        if im_type == "t9" then
            env.is_t9 = true
        end
    end
    ...
    -- [ToneFallback] 声调容错
    env.tone_state = "idle"
    env.lookup_key = config:get_string('sanhe_lookup/key') or '`'
...
local function handle_backspace(key, env, ctx)
    ...
    if env.bs_sequence then
        -- 不是移动设备，并且不属于特殊桌面环境
        if not sanhe.is_mobile_device() and not sanhe.is_special_desktop() then
...
local function handle_select_character(key, env, ctx)
    ...
    if sanhe.is_function_mode and sanhe.is_function_mode(ctx) then
        return false
    end
    -- 状态检查：必须在输入中或有候选菜单
...
local function handle_number_logic(key, env, ctx)
    ...
    if digit_str then
        ...
        if env.enable_tone_fallback then
            ...
            if sanhe.is_function_mode then
                is_func_mode = sanhe.is_function_mode(ctx)
            end
...
function M.func(key, env)
    ...
    if key:release() then 
        ...
        return sanhe.RIME_PROCESS_RESULTS.kNoop 
    end
    ...
    -- [Predict Space] 联想空格
    if kc == 0x20 then
        if handle_predict_space(key, env, ctx) then return sanhe.RIME_PROCESS_RESULTS.kAccepted end
    end

    -- 2. QuickSymbol 拦截 (a-z + /)
    if handle_quick_symbol_intercept(key, env, ctx) then
        return sanhe.RIME_PROCESS_RESULTS.kAccepted
    end

    -- 3. Backspace 退格防止删除已上屏内容
    if kc == 0xFF08 then
        if handle_backspace(key, env, ctx) then return sanhe.RIME_PROCESS_RESULTS.kAccepted end
    end
    ...
    if kc == 0x27 then
        if handle_segmentation(key, env, ctx) then return sanhe.RIME_PROCESS_RESULTS.kAccepted end
    end

    -- 6. 字母键 (a-z)[Limit Repeated] 重复输入限制
    if kc >= 0x61 and kc <= 0x7A then
        if handle_limit_repeat(key, env, ctx) then return sanhe.RIME_PROCESS_RESULTS.kAccepted end
    end

    -- 7. (q-o + 特定 Tag)[Letter Selector] 字母选词
    if env.ls_active and (LETTER_SEL_MAP[kc] ~= nil) then
        if handle_letter_select(key, env, ctx) then return sanhe.RIME_PROCESS_RESULTS.kAccepted end
    end

    -- 8. 数字键 (小键盘 + 声调 + 选词)[KpNumber & ToneFallback] 数字键综合逻辑
    if (kc >= 0xFFB0 and kc <= 0xFFB9) or (kc >= 0x30 and kc <= 0x39) then
        if handle_number_logic(key, env, ctx) then return sanhe.RIME_PROCESS_RESULTS.kAccepted end
    else
        -- 非数字键，重置声调状态
        if env.enable_tone_fallback then
            env.tone_state = "idle"
