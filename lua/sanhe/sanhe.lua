...
---@author amzxyz
---@return boolean
function sanhe.is_mobile_device()
...
---@return boolean
function sanhe.is_special_desktop()
...
---@param env Env
---@return boolean
function sanhe.is_pro_scheme(env)
    -- local schema_name = env.engine.schema.schema_name
    -- return schema_name:gsub("PRO$", "") ~= schema_name
    return env.engine.schema.schema_id == "sanhe_pro"
end

-- 以 `tag` 方式检测是否处于反查模式
function sanhe.is_in_radical_mode(env)
    local seg = env.engine.context.composition:back()
    return seg and (
        seg:has_tag("sanhe_reverse")
    ) or false
end
...
---@return boolean
function sanhe.is_function_mode(context)
...
---@param context Context | nil
---@return boolean
function sanhe.is_special_mode(context)
    if not context or not context.composition or context.composition:empty() then
        return false
    end
...
---判断文件是否存在
function sanhe.file_exists(filename)
...
-- 判断字符是否为汉字
function sanhe.IsChineseCharacter(text)
...
function sanhe.get_filename_with_fallback(filename)
    ...
    if sanhe.file_exists(user_path) then
        return user_path
    end
    ...
    if sanhe.file_exists(shared_path) then
        return shared_path
    end
    return nil
end
...
function sanhe.load_file_with_fallback(filename, mode)
    mode = mode or "r" -- 默认读取模式

    local _filename = sanhe.get_filename_with_fallback(filename)
...
---@return SanheRegexMatcher|nil matcher
---@return string|nil error_message
function sanhe.compile_regex(pattern)
...
---@param matcher SanheRegexMatcher|nil
---@param input string
---@return boolean
function sanhe.regex_matches(matcher, input)
...
function sanhe.get_input_method_type(env)
            ...
            if result_id == "unknown" then
                for j = 1, #INPUT_METHOD_MARKER_ORDER do
                    ...
                    if rule:find(symbol, 1, true) then
                        result_id = sanhe.INPUT_METHOD_MARKERS[symbol]
                        break
                    end
                end
            end
